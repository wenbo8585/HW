...
class GradientDescent(object):
    ...
    def _check_data(self, X, y):
        if X.shape[0] != y.shape[0]:
            raise ValueError("shape of X and y don't match")
    ...
    def train(self, X, y, init_wt = np.array([]), init_b=0, rate=0.01, alpha=0, epoch=1000,
              batch=None, validate_data=None):
        if init_wt.size == 0:
        ...
        if validate_data:
            for i in range(epoch):
                ...
                print("Epoch {:5d}: err = {:.6f} validate = {:.6f}".format(
                    i + 1, self.err_insample(), self.err(validate_data[0], validate_data[1])))
        else:
            for i in range(epoch):
                for j in range(tot_batch):
                    batch_X = self.__X[j * batch:min(num_data, (j + 1) * batch), :]
